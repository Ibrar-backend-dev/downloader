 ...
 */
async function accessArgs(url, log) {
  ...
  if (process.env.YTDLP_COOKIES_FROM_BROWSER) {
    ...
    if (log) log.info('cookies.enabled', { source: 'browser', value: process.env.YTDLP_COOKIES_FROM_BROWSER });
  } else if (process.env.YTDLP_COOKIES_FILE) {
    args.push('--cookies', process.env.YTDLP_COOKIES_FILE);
    if (log) log.info('cookies.enabled', { source: 'file', value: process.env.YTDLP_COOKIES_FILE });
  } else if (log) {
    log.warn('cookies.absent', { note: 'TikTok request will run unauthenticated' });
  }

  return args;
}

module.exports = { isTikTokUrl, accessArgs };
